	...
	K: keychain::Keychain + 'static,
{
	...
	let init = |init_args: InitTxArgs| match tx_flow {
		TxFlow::Atomic => owner_api.init_atomic_swap(keychain_mask, init_args),
		_ => owner_api.init_send_tx(keychain_mask, init_args),
	};
	...
	controller::foreign_single_use(
		...
		|api| {
			slate = match tx_flow {
				...
				_ => api.receive_tx(&slate, Some(&g_args.account), None)?,
			};
			Ok(())
		},
	)?;
	...
	K: keychain::Keychain + 'static,
{
	...
	slate = owner_api.countersign_atomic_swap(&slate, keychain_mask, None)?;
	if let (Some(dest), Some(tc)) = (ret_address.as_ref(), tor_config.as_ref()) {
		if !test_mode && tc.send_tor(args.skip_tor) {
			match try_slatepack_sync_workflow(&slate, dest, Some(tc.clone()), None, true) {
				Ok(_) => return Ok(()),
				Err(e) => error!("Error sending slate sync: {}", e),
			}
		}
	}
	output_slatepack(
	...
	K: keychain::Keychain + 'static,
{
	...
	slate = owner_api.process_multisig_tx(keychain_mask, &slate)?;
	if let (Some(dest), Some(tc)) = (ret_address.as_ref(), tor_config.as_ref()) {
		if !test_mode && tc.send_tor(Some(args.skip_tor)) {
			match try_slatepack_sync_workflow(&slate, dest, Some(tc.clone()), None, true) {
				Ok(_) => return Ok(()),
				Err(e) => error!("Error sending slate sync: {}", e),
			}
	...
	K: keychain::Keychain + 'static,
{
	...
	if is_invoice {
		...
		slate = match tx_flow {
			TxFlow::Atomic => owner_api.finalize_atomic_swap(keychain_mask, &slate)?,
			_ => owner_api.finalize_tx(keychain_mask, &slate)?,
		};
	}
