...
use crate::libwallet::{
	...
	TxFlow, TxLogEntry, ViewWallet, WalletInfo, WalletInst, WalletLCProvider,
};
	...
	K: Keychain,
{
	...
	///
	/// use std::sync::Arc;
	/// use util::{Mutex, ToHex, ZeroingString};
	///
	/// use api::Owner;
	...
	pub fn init_atomic_swap(
		...
		let slate = {
			let mut w_lock = self.wallet_inst.lock();
			let w = w_lock.lc_provider()?.wallet_inst()?;
			owner::init_atomic_swap(w, keychain_mask, args, self.doctest_mode)?
		};
		if let Some(sa) = send_args {
			let tc = crate::tor_config::load(&self.config_path())?;
			if !self.doctest_mode && tc.send_tor(sa.skip_tor) && !sa.dest.is_empty() {
				let dest = SlatepackAddress::try_from(sa.dest.as_str())?;
				return Ok(
					try_slatepack_sync_workflow(&slate, &dest, Some(tc), None, false)
						.unwrap_or(slate),
				);
			}
		}
	...
	pub fn countersign_atomic_swap(
		...
		let slate = {
			let mut w_lock = self.wallet_inst.lock();
			let w = w_lock.lc_provider()?.wallet_inst()?;
			owner::countersign_atomic_swap(w, slate, keychain_mask)?
		};
		if let Some(a) = r_addr {
			let dest = SlatepackAddress::try_from(a.as_str())?;
			let tc = crate::tor_config::load(&self.config_path())?;
			if !self.doctest_mode && tc.send_tor(None) {
				return Ok(
					try_slatepack_sync_workflow(&slate, &dest, Some(tc), None, true)
						.unwrap_or(slate),
				);
			}
		}
	...
	pub fn finalize_atomic_swap(
		...
		let mut w_lock = self.wallet_inst.lock();
		let w = w_lock.lc_provider()?.wallet_inst()?;
		owner::finalize_atomic_swap(w, keychain_mask, slate)
	}

	/// Recover and store the atomic key from the confirmed kernel
	...
	/// Explicitly export the local and recovered atomic keys
	/// `amount` is only needed to derive keys from legacy swaps
	pub fn get_atomic_secrets(
		...
		let our_nonce = match w.find_atomic_secret(keychain_mask, &atomic_id)? {
			Some(secret) => secret,
			None => keychain.derive_key(amount, &atomic_id, SwitchCommitmentType::Regular)?,
		};
		let rec_nonce = w.get_recovered_atomic_secret(keychain_mask, &atomic_id)?;
		Ok((our_nonce, rec_nonce))
	}
